...
@Query(value = "SELECT * FROM Sucursales JOIN Bodega b ON s.id = b.id_sucursal JOIN Producto p ON b.id_producto = p.codBarras WHERE (p.codBarras = :productoId OR p.nombre = :productoNombre) AND b.cantidad_disponible > 0)", 
                nativeQuery = true)
